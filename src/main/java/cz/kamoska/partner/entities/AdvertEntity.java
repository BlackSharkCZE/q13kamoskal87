...
@Table(name = "advert")
@NamedQueries({
	 @NamedQuery(name = "AdvertEntity.findAllActive", query = "SELECT a FROM AdvertEntity a WHERE a.state=cz.kamoska.partner.enums.AdvertState.ACTIVE AND a.bundleEntity.status = cz.kamoska.partner.enums.AdvertState.ACTIVE  ORDER BY a.acceptDate DESC"),
